...
function focusEntry(entry){if(!entry)return;updateConstellationPanel(entry);const ra=entry.ra/24*Math.PI*2,dec=THREE.MathUtils.degToRad(entry.dec);targetYaw+=shortestAngleDelta(targetYaw,-ra);targetPitch=dec;targetFov=Math.min(targetFov,46);history.replaceState(null,"",`?slug=${encodeURIComponent(entry.slug)}`);renderNav()}
function setZoom(delta){targetFov=THREE.MathUtils.clamp(targetFov+delta,24,104)}
function resetView(){targetYaw=0;targetPitch=0;targetRoll=0;targetFov=76;updateConstellationPanel(null);const url=new URL(location.href);url.searchParams.delete("slug");history.replaceState(null,"",`${url.pathname}${url.search}${url.hash}`);renderNav()}
