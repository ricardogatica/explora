...
export function createMilkyWayObject(galaxy,{detail=false}={}){
  ...
  const bar=new THREE.Mesh(new THREE.CylinderGeometry(detail?12:13,detail?12:13,detail?310:310,32,1,true),new THREE.MeshBasicMaterial({color:0xffe1b6,transparent:true,opacity:detail ? .22 : .12,blending:THREE.AdditiveBlending,depthWrite:false,side:THREE.DoubleSide}));
  bar.rotation.z=Math.PI/2;bar.rotation.y=.28;bar.userData.visibleFrom=galaxy.visibleFrom;group.add(bar);
  ...
  if(detail)group.add(makeBackgroundGalaxies());
  group.rotation.z=detail ? -.34 : -.16;
  group.scale.set(detail?1.24:1.05,detail ? .78 : .92,1);
